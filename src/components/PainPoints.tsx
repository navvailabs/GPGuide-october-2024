    ...
    return (
        <section id="features" className="relative overflow-hidden py-16 md:py-24 bg-brand-bg">
            ...
            <div className="relative z-10 container mx-auto px-4 sm:px-6 lg:px-8">
                <InspiredCard className="max-w-5xl mx-auto p-6 md:p-10">
                    <motion.div 
                    ...
                    <div className="relative max-w-3xl mx-auto">
                        {/* Vertical line */}
                        <div className="absolute left-4 md:left-1/2 top-0 h-full w-0.5 bg-brand-border dark:bg-white/10" />
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="absolute top-0 left-4 md:left-1/2 -translate-x-1/2 mt-1.5 z-10">
                                            <div className="w-5 h-5 rounded-full bg-brand-surface dark:bg-[#1A1B1E] border-2 border-brand-accent flex items-center justify-center">
                                        ...
                                        <div className={`w-full pl-12 md:pl-0 ${isRightSide ? 'md:pl-[calc(50%+2.5rem)]' : 'md:pr-[calc(50%+2.5rem)]'}`}>
                                            <motion.div 
                                                className="bg-gray-100 dark:bg-black/20 shadow-clay-light dark:shadow-clay-dark rounded-2xl p-6 transition-all duration-300 border border-transparent dark:border-white/5"
                                                whileHover={{ y: -5, scale: 1.02, transition: { type: 'spring', stiffness: 300 } }}
                                            >
                                                <div className="flex flex-col gap-4 text-left">
                                                    <div className="flex items-center gap-4">
                                                        <div className="w-12 h-12 flex-shrink-0 flex items-center justify-center rounded-lg p-2 shadow-md bg-brand-bg dark:bg-black/30 border border-brand-border dark:border-white/10">
                                                            <Icon className="text-brand-accent w-7 h-7" strokeWidth={1.5} />
                                                        </div>
                                                        <h3 className="text-lg font-bold leading-tight text-brand-text dark:text-gray-100">
                                                            {point.title}
                                                        </h3>
                                                    </div>
                                                    <ul className="space-y-3 text-sm">
                                                        <li className="flex items-start gap-2.5">
                                                            <div className="mt-0.5 flex-shrink-0">
                                                              <XCircle className="w-4 h-4 text-brand-text-muted dark:text-gray-400" />
                                                            </div>
                                                            <p className="text-brand-text-muted dark:text-gray-400">{point.pain}</p>
                                                        </li>
                                                        <li className="flex items-start gap-2.5">
                                                            <div className="mt-0.5 flex-shrink-0">
                                                              <CheckCircle className="w-4 h-4 text-brand-accent" />
                                                            </div>
                                                            <p className="font-semibold text-brand-text dark:text-gray-200">{point.gain}</p>
                                                        </li>
                                                    </ul>
                                                </div>
                                            </motion.div>
